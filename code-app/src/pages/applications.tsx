...
export default function ApplicationsPage() {
  ...
  // Everyone at the same company. This is what the interaction picker offers, so a
  // colleague can be logged against the application before being formally linked to
  // it — contacts.tsx scopes its application picker the same way, deliberately.
  ...
  // Only the contacts actually linked through the association table. The detail view
  // must use this: showing every colleague at the company as an "associated contact"
  // makes the association records look like they were never saved.
  const viewingApplicationAssociatedContacts = viewingApplication
    ? (() => {
        const linkedIds = new Set(getApplicationContactIds(viewingApplication.id));
        return contacts.filter((contact: NetworkingContact) => linkedIds.has(contact.id));
      })()
    : [];
